...
\noindent
The basic function of this module is to find the delay where the correlation coefficient is at a maximum at each joint. This is then averaged out over the number of joints taken into consideration to give an estimated delay. Finally, the signals for each of the joints of the student are shifted by the delay amount so that the signals are synchronised to the teacher. 
\subsubsection{Algorithm Description}
\noindent
\textbf{Not Done}
...
\subsubsection{Testing}
\noindent
The module has to pass various tests to be considered functional and reliable. To asses the performance, one subject is required to perform a simple ``dance'' using all limbs and then the same subject is required to repeat the moves but with different performance levels. The same subject is chosen to keep the experiment fair to as much of an extent as is possible. These repetitions have been divided into 4 different cases shown below. 
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.2]{Data_Analysis_Overall_Score.jpg}
\includegraphics[scale=0.2]{Data_Analysis_Four_Cases.jpg}
