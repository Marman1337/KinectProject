...
\noindent 
With the current design in mind the student will be asked to first move slightly so that the \texttt{NiTE} software can calibrate and start tracking. After this, the student will be required to stand still in a predefined position for a minimum amount of time, to be decided later, after which the student may start dancing. This will be the position that the next module will use to translate the coordinates with respect to a specific origin. The purpose of this module is to find a frame number during which the student will be in the standing still phase. 
...
\caption{Example showing signal jitter.}
\label{jitter}
\end{figure}
...
\noindent
The appearance of each dialog box has been designed by focusing on the end user, and the decision was actively taken to tailor the content within each dialog box to be simple to understand and control.  There are several examples of particular choices taken that demonstrate this.
There is a mechanism by which the user can watch tutorial videos that detail knowledge and specify how to use each feature of the application.
...
Within the mode that compares the skeletons of the student and teacher, included is a slider that acts as a media control, so that the user can move the position of the sliderís pointer essentially rewinding or forwarding the content on the screen, providing additional flexibility in the interaction between the user and the application.
...
\noindent
For all the main modes of the application, parts of the content displayed can be switched on or off by the user using a click of a button. This results in the ability to control aspects of the image, such as the background of a recording, the tracked skeleton within the augmented reality mode and either the student or teacher skeleton within the automated score mode.
Also integrated is a function which display a graph plotting the studentís average score for each window in the form of a bar chart, so that the user, for example, can locate which window has a particular low score and can return to play content from this specific window with just two clicks.
...
% Choosing another 2nd order arbitrary function which has a max at 0 and a min at x = 1.
%  Adding 
\noindent 
...
%   If a dance is too fast, high errors will be incurred.
%   Can't track dances in which there is too much interaction between 2 people e.g. salsa.
%   Student cannot be too short or tall compared to teacher or may need scaling module also. 
