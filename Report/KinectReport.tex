...
\subsubsection{Motionless Activity Detector}
\noindent 
...
\setlength{\parsep}{0pt}
	...
	\item Find the window with the minimum for the sum of absolute difference between frames.
...
The first step is to check that the algorithm works for only one joint, chosen to be the right hand. The subject moves the right hand until the \texttt{UserViewer} calibrates, then holds the hand in a certain position for a minimum of 1 second, and then moves it again. 
\subsubsection{Translation Module}
\subsubsection{Scaling Module}
\subsubsection{Initial Frame Detector}
