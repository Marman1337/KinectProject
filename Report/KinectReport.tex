...
\setlength{\parsep}{0pt}
	...
	\item Calculate the simple error for 21 different time lags $l \in [-10;10]$ according to the following formula:
	$$SimpleError = \dfrac{\sum\limits_{n=1}^{len}(t_s(n)-s_s(n))^2}{len \sum\limits_{j \epsilon J}
	\sum\limits_{i=x, y, z}|m_{t,j}(i)|}$$

	where $len$ is the number of samples in the shorter signal, $t_s$ and $s_s$ are the teacher's and student's signals with their mean subtracted, $J$ is the sum of all skeleton joints and $m_{t,j}(i)$ is the mean of the $i_{th}$ coordinate of the $j_{th}$ joint of the teacher.
	\item For the time lag with the lowest $SimpleError$, perform the windowed analysis, which consists of splitting the signal into variable number of equal parts, and calculating the previously described score for each part and joint separately in order to give more insightful results.
\end{itemize}

\subsubsection{Criteria for Scoring}
\subsubsection{Testing \& Results}
\subsubsection{Obstacles and Challenges}
\begin{itemize}
\item{Case 1: Student correctly repeats the dance.}
\item{Case 2: Student correctly repeats for a large portion of the dance but makes a single mistake with only a few joints.}
\item{Case 3: Student starts off okay but progressively gets worse.}
\item{Case 4: Student makes dramatic errors throughout.}
\end{itemize}

\noindent
The purpose of these test cases is to measure the effectiveness of the algorithm in different scenarios. First and foremost, the module should give a better score ( a lower error ) for students that more closely represent the dance as performed by the teacher. Furthermore, the algorithm should be able to identify upon receiving a low score,  the set of joints which is mostly responsible and the time frame in which the errors occured. It should also be able to give a very low score for anything that does not represent the dance at all, in all time frames. The tests will also help us to determine a threshold value for the score that is presented to the user, since the error is not a good measure as it is not bounded in any reasonable way. 
...
\noindent
One of the first errors in the initial code is that the torso automatically has a higher normalised error than any other body part for the same absolute error. This is due to the fact that the torso is centred about 0, since it is considered the origin, and therefore when diving the sum of the error squared by the sum of the teacher's torso coordinate values squared any small error is hugely magnified in comparison to the same absolute error in the hand coordinate values for instance. This has been rectified however by first subtracting the mean of the signals before comparing them. In this way all the signals are centred about zero and any similar absolute error is treated in a much fairer way. The initial offset is also saved so that, if necessary, the initial starting positions can be compared and a weighting for the score can be given to that also.   
\\\\
\noindent
Another problem that persisted is that, if the student performs well for a large portion of the dance but makes a huge error at some point, this information was not conveyed as an average was taken for the score and a single value was returned. This has been fixed however with the return of another vector of scores for each joint, in which the x, y, and z coordinates of the joint are each given a score and an average is taken. Additionally, this has been further improved by windowing the signal and giving a score for each joint and an average score for a specific time window. In this way the student can now know at what time period they have made an error and what body part has to improve. An example of such a case can be seen in Figure \ref{error_foot}. In this scenario the student performs two major errors which are reflected in the score of window 7 and 9. The window size (time period) can be determined by the teacher according to how high a resolution is necessary for the dance.
\\\\
\noindent
Another issue is the fact that though the error may be considered a plausible measure for the correctness of a dance to an engineer or a scientist, this may not be the case for an artist such as the dance teacher. Even if the error squared or the correlation is a good enough measure, the relationship may not be linear and so some care must be taken when finding these thresholds.

\begin{figure}[h]
\centering
\includegraphics[scale=0.3]{Data_Analysis_Error_Right_Foot.jpg}
\caption{Right foot x-coordinate data of teacher and student. As it can be seen the student makes 2 errors, the greatest of which having a peak at about 70\% of the way.}
\label{error_foot}
\end{figure}

\subsubsection{Results}
\noindent
The graphs in Figure \ref{error_score} show the results of test cases 1 to 4. As it is clear, the more closely the student represents the dance, the lower the error. This is both in the average score case, and in the time framed case. In Case 2 it can be seen that in two of the windows there is a large error, whereas in the rest of them the error is comparable to that of Case 1. This follows the video data as the student makes two errors with the arms within the periods of time that are represented by the windows. Added to that, we can see that in Case 3 the student starts off okay in the first half but in the second half the error increases greatly and as a consequence of the increased deviation from the teacher's dance. Finally, Case 4 shows an unacceptable error for all of the window frames as predicted. 
...
\caption{Overall error of the four test cases (top) and error for four test cases split into 10 portions of time (bottom).}
\label{error_score}
\end{figure}
